const turquoise = 'rgba(26, 188, 156, 1.0)';
const turquoiseDark = 'rgba(22, 160, 133, 1.0)';

const green = 'rgba(46, 204, 113, 1.0)';
const greenDark = 'rgba(39, 174, 96, 1.0)';

const blue = 'rgba(52, 152, 219, 1.0)';
const blueDark = 'rgba(41, 128, 185, 1.0)';

const purple = 'rgba(155, 89, 182, 1.0)';
const purpleDark = 'rgba(142, 68, 173, 1.0)';

const yellow = 'rgba(241, 196, 15, 1.0)';
const yellowDark = 'rgba(243, 156, 18, 1.0)';

const orange = 'rgba(230, 126, 34, 1.0)';
const orangeDark = 'rgba(211, 84, 0, 1.0)';

const red = 'rgba(231, 76, 60, 1.0)';
const redDark = 'rgba(192, 57, 43, 1.0)';

const lightGray = 'rgba(236, 240, 241, 1.0)';
const lightGrayDark = 'rgba(189, 195, 199, 1.0)';
const lightGrayTranslucent = 'rgba(189, 195, 199, 0.25)';

const gray = 'rgba(149, 165, 166, 1.0)';
const grayDark = 'rgba(127, 140, 141, 1.0)';
const grayDarkDark = 'rgba(93, 102, 103, 1.0)';
const grayTranslucent = 'rgba(149, 165, 166, 0.75)';

const darkGray = 'rgba(52, 73, 94, 1.0)';
const darkGrayDark = 'rgba(44, 62, 80, 1.0)';

const white = 'rgba(255, 255, 255, 1.0)';
const whiteTranslucent = 'rgba(255, 255, 255, 0.5)';
const whiteTranslucentDark = 'rgba(255, 255, 255, 0.75)';

export default {
  turquoise,
  turquoiseDark,

  green,
  greenDark,

  blue,
  blueDark,

  purple,
  purpleDark,

  yellow,
  yellowDark,

  orange,
  orangeDark,

  red,
  redDark,

  lightGray,
  lightGrayDark,
  lightGrayTranslucent,

  gray,
  grayDark,
  grayDarkDark,
  grayTranslucent,

  darkGray,
  darkGrayDark,

  white,
  whiteTranslucent,
  whiteTranslucentDark,

  brand: turquoise,
  brandDark: turquoiseDark,

  // header
  headerTint: white,
  headerBackground: turquoise,

  // tabs
  tabActiveTint: turquoise,
  tabInactiveTint: gray,

  // card colors
  cardBlue: blue,
  cardRed: red,
  cardYellow: yellow,
  cardGray: grayDark,
};
